...
Portable expectations for AI coding agents on Sequence Trip. Enforced in repo via `.cursor/rules/agent-contract.mdc` and `.cursor/rules/ponytail.mdc`.
...
- Platform viz: keep `.tsx` / `.web.tsx` in sync; web reads `progressRef` for smooth frames
...
- [AGENTS.md](../AGENTS.md) — repo policies, architecture, testing pyramid
